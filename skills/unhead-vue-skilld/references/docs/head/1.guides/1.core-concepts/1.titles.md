...
Now that we understand why direct title manipulation won't work, let's use Unhead's [`useHead()`{lang="ts"}](/docs/head/api/composables/use-head) composable to set titles properly:
...
## How do I add a site name to all titles?
...
[Template params](/docs/head/guides/plugins/template-params) like `%s` act as placeholders that get replaced with your page title and separator.
...
Check out the [Template Params](/docs/head/guides/plugins/template-params) guide to get started.
...
Remembering how to use the meta tags can be annoying, so we can use the [`useSeoMeta()`{lang="ts"}](/docs/head/api/composables/use-seo-meta) composable to set these up.
