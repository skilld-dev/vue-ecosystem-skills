...
By default, tags are rendered in the document `<head>`{lang="html"} in a [specific order](#how-does-tag-sort-order-work) for optimal performance and compatibility.
...
Capo.js weights are automatically applied to tags to avoid Critical Request Chains. As
