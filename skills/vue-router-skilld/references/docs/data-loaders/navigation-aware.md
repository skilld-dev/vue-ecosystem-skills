...
`reroute()` changes the current navigation from within a data loader. It accepts the same arguments as the return value of a navigation guard **as long as it changes the navigation**. It doesn't accept `true` or `undefined` as these values do not change the navigation. It **throws internally** to immediately stop the loader execution.
