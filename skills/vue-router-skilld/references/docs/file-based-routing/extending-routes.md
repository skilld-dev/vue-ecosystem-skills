...
The `<route>` custom block is a way to extend existing routes. It can be used to add new `meta` fields, override the `path`, the `name`, or anything else in a route. **It has to be added to a `.vue` component inside of the [routes folder](./file-based-routing#routes-folder-structure)**. It is similar to the same feature in vite-plugin-pages to facilitate migration.
...
One example of this is using vite-plugin-vue-layouts which can only be used this way:
